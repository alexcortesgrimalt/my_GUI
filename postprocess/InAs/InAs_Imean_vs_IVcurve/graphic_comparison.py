...
if v_csv and i_csv:
    plt.plot(v_csv, i_csv, linestyle='-', linewidth=2.5, color='royalblue', 
             label='I(V) with Keithley', zorder=1)

# Mean points from TIFF maps & Resistance Calculation
if vbias_maps:
    # Ordenar los voltajes y corrientes de menor a mayor
    vbias_sorted, imean_sorted = zip(*sorted(zip(vbias_maps, imean_maps)))
    v_arr = np.array(vbias_sorted)
    i_arr = np.array(imean_sorted)
    
    # --- CÁLCULO DE RESISTENCIA ---
    # Ajuste lineal: I = m*V + c (m = Conductancia en nA/V)
    slope, intercept = np.polyfit(v_arr, i_arr, 1)
    
    # Convertimos la pendiente (nA/V) a Siemens (A/V) para calcular Ohmios
    conductance_S = slope * 1e-9
    r_kohm = (1 / conductance_S) / 1000
    
    # Dibujar la línea de tendencia del ajuste
    fit_line = slope * v_arr + intercept
    plt.plot(v_arr, fit_line, linestyle='--', color='crimson', alpha=0.6, linewidth=2,
             label=rf'EBIC Linear Fit ($R \approx {r_kohm:.1f} \ \mathrm{{k\Omega}}$)', zorder=2)

    # Dibujar los puntos reales de los mapas
    plt.scatter(v_arr, i_arr, marker='o', s=120, color='crimson', 
                edgecolor='black', label='Mean($I_{EBIC}(x,y)$)', zorder=3)
