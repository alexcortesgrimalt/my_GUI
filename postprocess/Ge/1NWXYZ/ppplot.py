...
for d, df, r in plot_data:
    label_str = fr'${d} \ \mu\mathrm{{m}}$'
    
    # Multiplicamos la corriente por 1e9 para mostrarla en nanamperios
    plt.plot(df['V'], df['I'] * 1e9, linewidth=2.5, label=label_str)

# Ejes con formato MathText estricto (etiqueta Y actualizada a nanamperios)
plt.xlabel(r'$V \ (\mathrm{V})$')
plt.ylabel(r'$I \ (n\mathrm{A})$')
